...
export default function DailyPlanPage() {
  ...
  const [user, setUser] = useState<{ region?: string } | null>(null)
  ...
  const fetchData = async (startDate?: string, endDate?: string, options?: { showFullLoading?: boolean; region?: string }) => {
    try {
      if (options?.showFullLoading !== false) setLoading(true)
      
      // Build URL with date and region parameters
  ...
  // Initial data fetch with default date range (1 week back to tomorrow)
  useEffect(() => {
    // Load optimized dataset (7 days back + today + tomorrow) to reduce bandwidth
    fetchData(dateFilter.startDate, dateFilter.endDate, { region: user?.region })
  }, [user?.region]) // Re-fetch when user region changes

  // Check if date range is within loaded data range
  const isWithinLoadedRange = (startDate: string, endDate: string) => {
    // Current loaded range: 7 days back to tomorrow from initial load
    const currentStart = new Date(dateFilter.startDate)
    const currentEnd = new Date(dateFilter.endDate)
    const requestedStart = new Date(startDate)
    const requestedEnd = new Date(endDate)
    
    // If requested range is within or equal to loaded range, use client-side filtering
  ...
  const handleDateFilterChange = async (newDateFilter: { startDate: string; endDate: string }) => {
    ...
    if (!isWithinLoadedRange(newDateFilter.startDate, newDateFilter.endDate)) {
      ...
      await fetchData(newDateFilter.startDate, newDateFilter.endDate, { showFullLoading: false, region: user?.region })
      setTableRefreshing(false)
    }
    // Otherwise, DailyPlanTable will handle client-side filtering automatically
  }

  // Fetch user info to get region
  useEffect(() => {
  ...
  const handleRefresh = async () => {
    ...
    await fetchData(dateFilter.startDate, dateFilter.endDate, { showFullLoading: false, region: user?.region })
    setTableRefreshing(false)
  }
