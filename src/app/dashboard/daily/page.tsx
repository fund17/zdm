        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {dashboardCards.map((card, index) => {
            ...
            return (
              <div 
                key={index} 
                className={`${(card as any).color} overflow-hidden shadow-sm rounded-xl border-2 ${(card as any).flipBorder} hover:shadow-md transition-all duration-300 ${hasDetail ? 'cursor-pointer' : ''}`}
                onClick={() => hasDetail && setZoomedStatsCard(index)}
              >
                <div className="p-3 flex flex-col h-full min-h-[110px]">
                  <div className="flex items-start gap-3">
                    <div className="flex-shrink-0">
                      <div className={`w-10 h-10 ${(card as any).iconBg} rounded-lg flex items-center justify-center shadow-sm`}>
                        <IconComponent className="h-5 w-5 text-white" />
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <dt className={`text-[10px] font-semibold ${(card as any).textColor} uppercase tracking-wide truncate`}>{card.title}</dt>
                      <dd className="flex items-baseline mt-0.5">
                        <div className={`text-2xl font-bold ${(card as any).textColor}`}>
                          {loading ? '...' : card.value}
                        </div>
                      </dd>
                      <dd className={`text-[10px] ${(card as any).textColor} mt-0.5 font-medium opacity-70 truncate`}>{card.change}</dd>
                    </div>
                  </div>
                  {/* Click hint */}
                  {hasDetail && (
                    <div className={`mt-auto pt-2 border-t ${(card as any).flipBorder}`}>
                      <div className={`text-[9px] font-semibold ${(card as any).textColor} flex items-center justify-center gap-1 opacity-75`}>
